...
async function fetchPopularMovies() {
    try {
        ...
        const data = await response.json();// tu codigo aqui: realiza una solicitud para obtener las películas populares
...
async function showMovieDetails(movieId) {
    try {
        ...
        const movie = await response.json(); // tu codigo aqui: realiza una solicitud para obtener los detalles de la película
        ...
        selectedMovieId = movieId; // Guarda el ID de la película seleccionada para agregarla a favoritos
        movieDetails.classList.remove('hidden');// y actualiza el contenedor de detalles con la información de la película
    } catch (error) {
...
searchButton.addEventListener('click', async () => {
    ...
    if (query) {
        try {
            ...
            const data = await response.json();// tu codigo aqui: realiza una solicitud para buscar películas
            displayMovies(data.results);// y llama a displayMovies con los resultados de la búsqueda
        } catch (error) {
